...
// StaticResolver is an optional interface that FieldResolvers can implement to
// enable exact short-circuit evaluation of OR branches that only reference
// static request data (e.g. @request.auth.* fields).
//
// Implementations must evaluate expressions with the same semantics as the SQL
// generated by BuildExpr. The second return value must be false if the
// expression depends on record data or anything else that cannot be evaluated
// as a standalone scalar SQL expression.
//
// ValidateStaticExpr must also preserve any resolver side effects from the
// original expression because relation joins and list-rule checks are part of
// the final query semantics.
type StaticResolver interface {
	EvaluateStaticExpr(data []fexpr.ExprGroup) (matched bool, ok bool, err error)
	ValidateStaticExpr(data []fexpr.ExprGroup, maxExpressions int) error
}
