...
func (f FilterData) BuildExprWithLimit(
	...
	if sr, ok := fieldResolver.(StaticResolver); ok {
		result := tryShortCircuitOr(data, sr)
		if result.passed || result.remaining != nil {
			if err := sr.ValidateStaticExpr(data, maxExpressions); err != nil {
				return nil, err
			}
		}
		switch {
...
// BuildParsedFilterExpr builds an already parsed filter expression without
// running the static OR short-circuit optimizer.
